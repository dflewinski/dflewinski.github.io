...
function infoDescriptionOnCreate (){
    return '<button class="collapsible"><div><h4 style="width: 100%;">Koszty lekcji religii w powiatach i miastach województwa wielkopolskiego</h4></div></button>' +
        '<div class="content">' +
        '<p>Nie zawiera kosztów ponoszonych przez gminy (np. szkoły podstawowe).</p>' +
...
var STATUS = L.geoJson(powiatyData, {style: styleStatus, onEachFeature: STATUSonEachFeature});
var COST = L.geoJson(powiatyData, {style: styleCost, onEachFeature: COSTonEachFeature});
var PERSTUDENT = L.geoJson(powiatyData, {style: stylePerStudent, onEachFeature: PERSTUDENTonEachFeature}).addTo(map);
var PERCITIZEN = L.geoJson(powiatyData, {style: stylePerCitizen, onEachFeature: PERCITIZENonEachFeature});
var POWIATCOST = L.geoJson(powiatyData, {style: styleGminaCost, onEachFeature: POWIATCOSTonEachFeature});
var SUBVENTION = L.geoJson(powiatyData, {style: styleSubvention, onEachFeature: SUBVENTIONonEachFeature});
...
function getColorByPowiatCost(d) {
...
function styleGminaCost(feature) {
    return {
        ...
        dashArray: '3',
        fillOpacity: 0.7,
        fillColor: getColorByPowiatCost(feature.properties.gminaCost)
    };
}
...
var POWIATCOSTlegend = L.control({position: 'bottomright'});
POWIATCOSTlegend.onAdd = function (map) {
    var div = L.DomUtil.create('div', 'info legend'),
        grades = [
            120001,
            100001,
    ...
    div.innerHTML += '<div><b>Roczne wydatki z budżetu powiatu</b></div><br>'

    // loop through our density intervals and generate a label with a colored square for each interval
    for (var i = 0; i < grades.length; i++) {
        div.innerHTML +=
            '<i style="background:' + getColorByPowiatCost(grades[i]) + '"></i> ' +
            labels[i] + '<br>';
    }
    return div;
};
...
map.on('baselayerchange', function (eventLayer) {
    ...
    } else if (eventLayer.name === 'Koszt powiatu') {
        map.removeControl(currentLegend);
        currentLegend = POWIATCOSTlegend;
        POWIATCOSTlegend.addTo(map);
...
function POWIATCOSTonEachFeature(feature, layer) {
    layer.bindPopup(getDescriptionPopup(feature));
    layer.bindTooltip(getDescription(feature));
